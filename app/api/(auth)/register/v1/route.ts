...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const [existingUser] = await conn.execute("SELECT * FROM users WHERE user_email = ? OR user_name = ?", [data.email, data.username]);
        const result = existingUser as UserTableType[];
        if (result.length > 0) {
            const text = result[0].email === data.email ? "Email" : "Username";
            const codeText = result[0].email === data.email ? "EMAIL_EXISTS" : "USERNAME_EXISTS";
            return response(null, `${text} already exists`, codeText, 409);
        }

        const [insertResult] = await conn.execute<ResultSetHeader>("INSERT INTO users (username, email, password, gender) VALUES (?, ?, ?, ?)", [data.username, data.email, data.password, data.gender]);
