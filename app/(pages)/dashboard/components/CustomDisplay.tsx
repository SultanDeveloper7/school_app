...
import CustomTableView from "./displayItem/CustomTableView";
...
export default function CustomDisplay(props: CustomDisplayProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#2e2d2d]">
      <HeaderItem permissions={permissions} gridColDef={columns} />
    </div>
  );
}
...
function HeaderItem({ permissions, gridColDef }: HeaderItemProps) {
  const subPermissions = permissions?.sub_permissions;
  return (
    <div className="flex h-full flex-col bg-[#242424]">
      ...
      <div className="flex-1 min-h-0">
        <CustomTableView gridColDef={gridColDef} permission={permissions?.permission_code} />
      </div>
    </div>
  );
}
